<template>
    <div class="px-5 has-background-grey-light" style="border-radius: 0 0 30px 30px;">
        <figure class="image is-128x128" style="margin: -20% 30% 0 30%;">
            <img class="is-rounded" src="~/assets/images/dp_pg.png">
        </figure>
        <h3 class="title is-3 mt-5 has-text-primary">
            Boluwaji AKINSEFUNMI <br />
            <span class="tag is-primary mb-5">Developer</span>
        </h3>
        <p class="subtitle has-text-black">
            <span class="has-text-grey-darker">Phone:</span><br />
            +234 810 341 2973
        </p>
        <p class="subtitle has-text-black">
            <span class="has-text-grey-darker">Email:</span><br />
            bolu.akinsefunmi@gmail.com
        </p>
        <p class="subtitle has-text-black">
            <span class="has-text-grey-darker">Website:</span><br />
            https://github.com/IAmMasterCraft
        </p>
        <p class="subtitle has-text-black">
            <span class="has-text-grey-darker">Location:</span><br />
            Lagos State
        </p>
        <h4 class="title is-4 mt-6 has-text-primary">ABOUT ME</h4>
        <p class="subtitle has-text-black pb-6">
            I am an easy-going and personally motivated individual with the ability to work as part of
            a team and independently on my own.
        </p>
    </div>
</template>